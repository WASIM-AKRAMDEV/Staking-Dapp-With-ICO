...
function CONVERT_TIME_TO_READABLE(timeStamp) {
  ...
  try {
    ...
    if (address) {
        ...
        notifications.map(
          async ({ poolID, amount, user, typeOf, timeStamp }) => {
            return {
              ...
              user: user,
              typeOf: typeOf,
              timeStamp: CONVERT_TIME_TO_READABLE(timeStamp),
            };
          }
        )
      );
      ...
      for (let i = 0; i < length; i++) {
        ...
        const userReward = await contractObj.userReward(i, address);
        ...
        const pool = {
          ...
          lockUtils: CONVERT_TIME_TO_READABLE(userInfo.lockUtils.toNumber()),
...
export async function deposit(poolID, amount, address) {
  try {
    ...
    const gasEstimatimation = await contractObj.estimateGas.deposit(
      Number(poolID),
      amountInWei
    );
...
export async function withdraw(poolID, amount) {
  try {
    ...
    notifySuccess("tramntions successfully completed");
    return receipt;
  } catch (error) {
    console.log(error);
    const errorMsg = parseErrorMsg(error);
...
export async function createPool(pool) {
  try {
    const { _depositeToken, _rewardToken, _apy, _lockDays } = pool;
    if ((!_depositeToken || _rewardToken || !_apy, !_lockDays))
      return notifyError("Provide all the detials");
    notifySuccess("calling contract...");
    ...
    const StackTx = await contractObj.addPool(
      _depositeToken,
      _rewardToken,
      Number(_apy),
      Number(_lockDays),
    ...
    const receipt = await StackTx.wait();
...
export async function modifyPool(poolID, amount) {
  try {
    ...
    const receipt = await data.wait();
    notifySuccess("Pool Mofify successfully");
    return receipt;
  } catch (error) {
    console.log(error);
    const errorMsg = parseErrorMsg(error);
...
export const addTokenMetaMask = async (token) => {
  if (window.ethereum) {
    const contarct = await tokenContract();
